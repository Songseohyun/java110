...
// 1) 웹 애플리케이션의 경로를 root(/),port:8888 로 설정했을 경우,
...
//   서블릿에 인스턴스 변수를 사용해서는 안된다.
//   why? Servlet 객체는 한 개만 생성되고 모든 Client 가 공유한다.
...
public class Servlet01 implements Servlet{
    ...
    public void init(ServletConfig config) throws ServletException {
        ...
        // 이 메소드에서는 서블릿이 작업할 때 사용할 자원을 준비하는 일을 둔다.
        // ex) DB connection 준비, 외부 서버에 소켓을 연결 등의 작업
    ...
    public ServletConfig getServletConfig() {
        ...
        // 이 메소드를 호출하여 ServletConfig 객체를 리턴 받아 사용한다.
        // 그래서 이 메소드는 init()에서 파라미터로 받은 ServletConfig 객체를 리턴해야한다.
        return this.config;
    }
    ...
    public void service(ServletRequest req, ServletResponse res)
        ...
        // 클라이언트가 이 서블릿의 샐행을 요청할 때마다 서블릿 컨테이너가 호출한다.
        // 그래서 클라이언트가 요청한 일을 수행한다.
        // ex) 게시물 목록을 출력하거나 로그인을 처리하거나 회원 가입을 처리하는 등의 일을 한다.
    ...
    public String getServletInfo() {
        ...
        // 이 메소드의 리턴값을 받아서 출력한다.
        return "Servlet01";
    }

    @Override
    public void destroy() {
        // 서블릿 객체가 제거되기 직전에 호출되는 메소드이다.
        // 그래서 이 메소드에서는 init()에서 준비한 자원을 해제시키는 일을 한다.
        // ex) DB connection을 close 하거나 소켓 연결을 close 등의 작업을 수행한다.
