...
public class Filter01 implements Filter{
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        //Client가 요청한 Servlet을 실행하기 전에 해당 URL에 등록한 Filter를 실행한다.
        // 이때 이 메소드가 호출된다.
        // 서블릿을 실행하기 전,후에 수행해야 할 작업이 있다면 이 메소드에서 실행한다.
        // 서블릿을 실행하기 전에 하는 일:
        ...
        // 위 메소드를 호출하지 않으면 다음 필터나 서블릿을 실행하지 않는다.
        // 따라서 빼면 안된다.
        // 최종적으로 서블릿의 service() 호출이 끝나면 위 메소드는 리턴된다.
    ...
    public void init(FilterConfig filterConfig) throws ServletException {
        // Filter가 생성된 후 생성자 호출 후 이 메소드가 실행된다.
        // Filter가 작업하는 동안 사용할 자원을 준비하는 일을 한다.
        System.out.println("Filter01.init() 호출됨.");
    }
    @Override
    public void destroy() {
        // tomcat 서버가 종료되거나 웹 애플리케이션이 멈출 때
        // 생성된 모든 필터에 대해 이 메소드를 호출한다.
